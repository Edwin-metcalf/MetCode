...
- run_command: Run a command with the given arguments. The command parameter command is one literal executable (ie go or grep) no subcommands or flags in this field. The arguments parameter is a list of subcommands/flags to pass, each as a separate element (ie ["run", "main.go"]). The path parameter is optional and sets the working directory the command is run in. If left out the command runs in the current directory. If the user declines to run the command you offer you will get back "command canceled by user". Use this when asked to run a command, or when running one would help complete the task.
